...
%To obtain the values of $c_1$, $c_2$ and $A$ some other conditions must be satisfied.Since finite $R = 0$ has already been enforced, consider a point at the border $R=R_0$. At the point $R_0$, the stream function must be the same as it was far upstream (streamlines cannot cross), i.e. $\psi(R_0) = \frac12  WR_0^2$
%Conditions
...
% \section{Outer vortex breakdown}
...
%look at euler equations to derive the detadt and dvdt equations (potentially)
%cylindrical euler equations
%better to just reference it though


%try higher order finite diff for the r=R bc of eta
%repeat the wang rusak flow stuff in the `wall separation' paper 
%will need to use a perturbation similar to what they use however.
%just above section 4.1
\begin{align*}
...
try playing around with outlet conditions. wang,rusak use the d*dz = 0

trent suggested at $z=Z$
\[\dd \phi t = -U_{a} \dd \phi z\]
where $\phi$ would be both v and $\eta$ and $U_a$ is a parameter
...
%I AM MISSING A W,R PAPER
%The dynamics of a swirling flow in a pipe and ...
