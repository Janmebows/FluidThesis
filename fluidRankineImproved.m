...
det =@(rhat,rstar) rhat*(bessely(1,k*rhat)*besselj(1,k*rstar) - besselj(1,k*rhat)*bessely(1,k*rhat));
...
rstar = x(2);
r= linspace(0,R);
Win = W + k*(A(rhat,rstar)*besselj(0,k*r) + B(rhat,rstar)*bessely(0,k*r));
...
rhat = x(1);
rstar = x(2);
det = rhat*(bessely(1,k*rhat)*besselj(1,k*rstar) - besselj(1,k*rhat)*bessely(1,k*rhat));
